...
echo "[4/5] checking robot_gps..."
GPS_RUNNING=$(_exec "ps aux | grep '[r]obot_gps' | grep -v grep | wc -l")
if [ "$GPS_RUNNING" -eq 0 ]; then
    echo "[4/5] starting robot_gps..."
    _exec "$ROS_SETUP && nohup ros2 run sensors robot_gps > /tmp/robot_gps.log 2>&1 &"
    sleep 1
else
    echo "[4/5] robot_gps already running — OK"
fi

# ── 5. Reset firmware → IDLE, then START → RUNNING ───────────────────────────
# Firmware boots in INIT state; set_state(RUNNING) only works from IDLE.
# RESET (cmd=3) transitions any state → IDLE; START (cmd=1) → RUNNING.
...
sleep 1
echo "[5/6] starting firmware to RUNNING..."
_exec "$ROS_SETUP && ros2 topic pub --once /sys_cmd bridge_interfaces/msg/SysCommand \
